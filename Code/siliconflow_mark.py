...
# --- 0. Config (Primary-topic whitelist) ---
...
class ModelChat(LLM):
    ...
    def __init__(self, api_secret: Optional[str] = None, model_name: Optional[str] = None):
        super().__init__()
        # Prefer explicit param; otherwise load from env
    ...
    # Network-level retry: keep 3 tries to handle transient network issues
    ...
    def _call(self, prompt: str, stop: Optional[List[str]] = None) -> Optional[str]:
        """Internal call with up to 3 retries for robustness."""
        for _ in range(3):
            try:
...
if __name__ == "__main__":
    ...
    # >>> Change output filename here to separate different runs/processes <<<
    OUTPUT_FILE_PATH = r"path/to/data.xlsx"

    print(f"Output file: {OUTPUT_FILE_PATH}")

    # Load API keys from environment (recommended).
    # Example (Linux/macOS): export SILICONFLOW_API_KEYS="key1,key2,key3"
    # Example (Windows PowerShell): setx SILICONFLOW_API_KEYS "key1,key2,key3"
    ...
    if "标签" not in df_target.columns:
        exit("Missing required column: 标签")
