...
# Strictly aligned with the topic set in the prompt
VALID_OPTIONS = [
    "自闭症症状",
    "自闭症检查",
    "自闭症诊断",
...
def clean_and_validate_label(raw_label):
    ...
    # 2) Prefix stripping (e.g., "1. xxx", "Topic: xxx")
    cleaned = re.sub(r"^[\d\w]+\.|^Category:|^\w+\s*-\s*|^Topic:", "", s_label).strip()
    if cleaned in VALID_OPTIONS:
        return cleaned

    # 3) Substring match (for verbose outputs like "主题：xxx")
...
# Prompt template (uses QA + text, but classifies ONLY based on text)
...
def process_chunk_worker(args):
    ...
    args: (df_chunk, api_secret, worker_id, qa_col_name)
    """
    df_chunk, api_secret, worker_id, qa_col_name = args
    llm = ModelChat(api_secret)
    df_chunk_processed = df_chunk.copy()

    for _, index in enumerate(df_chunk_processed.index):
        # 1) Read the main text to be classified
        if "患者需求" in df_chunk_processed.columns:
            text_val = df_chunk_processed.at[index, "患者需求"]
        elif "Content" in df_chunk_processed.columns:
            text_val = df_chunk_processed.at[index, "Content"]
        else:
            text_val = ""

        # 2) Read QA context (for context completion only; NOT for classification)
        ...
        # If the main text is empty, we cannot classify
...
if __name__ == "__main__":
    ...
    # Read API keys from environment variables (do not hardcode secrets).
    # Example:
    # export SILICONFLOW_API_KEYS="key1,key2"
    ...
    if not api_secrets:
        raise ValueError("No API keys found. Set SILICONFLOW_API_KEYS or SILICONFLOW_API_KEY.")
        ...
        try:
            with ThreadPoolExecutor(max_workers=num_workers) as executor:
                for batch_idx in index_batches:
                    ...
                    tasks = [
                        (chunks[i], api_secrets[i % num_workers], i + 1, QA_COLUMN_NAME)
